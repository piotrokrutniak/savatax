...
export default function PostList(){
    ...
    return(
        <div className="text-white flex flex-col gap-5">
            ...
            <div className="flex p-3 gap-2 select-none justify-between place-self-center"> <p>Loading results</p> <BsArrowClockwise className="animate-spin h-6 w-6"/></div> : 
            posts.map(x => <PostItem post={x}/>)}
            <div className="flex justify-center min-h-screen-1/2 w-full bg-black p-8 rounded-md gap-2">
                {loading &&<PaginationPanel page={page} setPage={setPage} pages={pages}/>}
            </div>
        </div>
    )
}