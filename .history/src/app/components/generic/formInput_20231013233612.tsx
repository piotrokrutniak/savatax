    ...
    return(
        <div className={`${className ?? ""} flex flex-col relative`}>
            ...
            <input type={type || "text"} defaultValue={defaultValue || ""} placeholder={placeholder ?? ""}
                ...
                onBlur={() => handleBlur()} 
                onFocus={() => handleFocus()}
                className={`${inputClassName ?? ""} p-3 w-full text-white text-base rounded-lg outline-none bg-black focus:bg-slate-700 border-2 border-transparent focus:border-sky-100/50
                transition-all`}/>
