    ...
    return(
        <div className="min-h-screen-3/4 flex flex-col justify-between w-full bg-black px-4 rounded-md gap-2">
            <div className="flex flex-col sm:grid sm:grid-auto-fit-xl gap-2">
                ...
                <LoadingPanel/> : 
                users.map((x: User, key: number) => <UserItem user={x} key={key}/>)}
            </div>
...
function UserItem({user}:{user:User}){
    return(
        <div className="w-full flex flex-col bg-slate-500/30 p-4 rounded-md">
            ...
            <p className="w-fit ml-auto mr-0 text-xs md:text-sm p-3 bg-black/30 rounded-md mt-2">{user?.company?.name}</p>
        </div>
    )
}