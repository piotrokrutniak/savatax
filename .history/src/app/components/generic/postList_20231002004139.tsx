...
function PaginationPanel({page = 1}:{page: number}){
    ...
    return(
        pages.map(x => 
            <div className={`${x == page ? "bg-sky-600" : "bg-slate-300/40"} h-8 w-8 aspect-square  flex justify-center items-center rounded-md cursor-pointer opacity-80 hover:opacity-100 active:opacity-60`}> 
                {x} 
            </div>
        )
    )
