...
export default function NavBar(){
    ...
    const [language, setLanguage] = useState(getURL().slice(1, 3))
            ...
            <div className="w-full flex">
                <div className="p-2 m-auto w-full flex max-w-7xl justify-between items-center">
                    ...
                    <div className="hidden text-lg gap-4 md:flex">
                        ...
                        <DropDown languages={languages} language={language}/>
                    </div>
...
function DropDown({languages, language}:{languages: any[], language: string}){
    ...
    return(
        <div className="relative">
            ...
            <div className="w-28 p-3 bg-black rounded-lg text-white flex place-items-center gap-2 cursor-pointer justify-between"
            ...
            <div className={`${opened ? "h-[88px]" : "h-0"} overflow-hidden w-44 bg-white text-black shadow-lg absolute top-16 right-0 rounded-lg transition-all`}>
                {languages.map(x => <DropdownOption setLabel={setLabel} label={x.label} value={x.code}/>)}
            </div>
        </div>
    )
}

function DropdownOption({value, label, setLabel}:{value: string, label: string, setLabel: Dispatch<SetStateAction<string>>}){
    return(
        <Link href={"/" + value}>
        <div className="p-2 hover:bg-slate-400/20 cursor-pointer" onClick={() => setLabel(label)}>{label}</div>
        </Link>
    )
}