...
export default function NavBar(){
    ...
    const language = "pl"
    const languages = {"pl": {code: "pl", label: "Polish"}, "en": {code: "en", label: "English"}}
    ...
    return(
        <div className="relative">
            ...
            <div className={`${opened ? "h-[88px]" : "h-0"} overflow-hidden w-44 bg-white text-black shadow-lg absolute top-16 right-0 rounded-lg transition-all`}>
                {languages.map(x => <DropdownOption setLabel={setLabel} label={x[language].label} value={x[language].code}/>)}
            </div>
        </div>
    )
}
