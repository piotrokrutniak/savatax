...
export default function NavBar(){
    ...
    const [language, setLanguage] = useState(getURL().slice(1, 3) ?? "pl")
    ...
    return(
        <div className="relative">
            ...
            <div className={`${opened ? "h-[88px]" : "h-0"} overflow-hidden w-44 bg-white text-black shadow-lg absolute top-16 right-0 rounded-lg transition-all`}>
                {languages.map(x => <DropdownOption setLabel={setLabel} label={x.label} value={x.code} setLanguage={setLanguage}/>)}
            </div>
        </div>
    )
}

function DropdownOption({value, label, setLabel, setLanguage}:{value: string, label: string, setLabel: Dispatch<SetStateAction<string>>, setLanguage: Dispatch<SetStateAction<string>>}){
    ...
    return(
        <Link href={"/" + value}>
        <div className="p-2 hover:bg-slate-400/20 cursor-pointer" onClick={() => HandleClick()}>{label}</div>
        </Link>
    )
}