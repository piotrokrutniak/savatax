...
export function PostItem({post}:{post: Post}){
      ...
      return(
        <div className="w-full p-4 bg-black hover:bg-slate-500/20 rounded-lg shadow-md overflow-clip shadow-black/40 flex flex-col lg:flex-row md:justify-between gap-4 items-center text-white group transition-colors">
          <div className='w-full aspect-square lg:w-96 h-80 shrink-0 rounded-lg overflow-hidden relative cursor-pointer flex justify-center place-items-center bg-black shadow-sm shadow-black/40'>
          ...
          <div className="h-fit items-center p-2">
            <div className="flex flex-col lg:flex-row w-full justify-between text-xl gap-2 lg:gap-5">
              <h1 className="flex place-items-center cursor-pointer w-full capitalize
              ...
              <div className="flex flex-row-reverse">
                ...
                <Link href={"/users/" + user?.id}>
                  ...
                  <h2 className="flex flex-row-reverse font-light text-xs sm:text-base">@{user?.username ?? "Guest"}</h2>
                </Link>
                </div>
              </div>
            </div>
