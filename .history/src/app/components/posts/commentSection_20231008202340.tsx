...
export default function CommentsSection({post, commentsRef, setCommentsOpen}:{
    post: Post;
    commentsRef: any;
    ...
    useEffect(() => {
        setCommentValidated(titleString.trim().length > 1 && commentString.trim().length > 1)
    ...
    return(
      <div className={`comments-${post.id} bg-slate-500/30 w-full overflow-hidden rounded-lg relative h-full`} id="comments-wrapper">
        <div id="comments-header" className="p-3 border-b-2 border-slate-400/20 shadow-sm shadow-black/40 w-full flex justify-between place-items-center" ref={commentsRef}>
        ...
        <div className="p-1 sm:p-4 min-h-screen-1/4 max-h-screen-2/3 flex flex-col gap-4 bg-slate-500/30 overflow-y-scroll">
          {loading ?
          (postComments.length > 0 ? postComments.map((x: Comment) => <CommentItem postId={post.id} comment={x}/>) : 
  ...
  function CommentItem({comment, postId}:{comment: Comment; postId: number;}){
    return(
      <div className={`comments-${postId} p-4 bg-black rounded-lg flex flex-col gap-1`}>
