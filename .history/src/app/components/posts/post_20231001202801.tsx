...
export function PostItem({post}:{post: Post}){
      ...
      return(
        <div className="w-full p-4 md:h-48 bg-black hover:bg-slate-500/20 rounded-lg shadow-md shadow-black/40 flex flex-col md:flex-row md:justify-between gap-4 items-center text-white group">
          ...
          <div className="h-fit items-center">
            <div className="flex flex-col md:flex-row w-full justify-between gap-2 text-xl">
              <h1 className="flex cursor-pointer bg-clip-text w-full whitespace-nowrap capitalize overflow-ellipsis overflow-hidden hover:text-transparent bg-gradient-to-r from-vermilion-500 to-vermilion-400">
                {post.title ?? "Post Title"}
              </h1>
              <div className="w-full flex-col">
                <h2 className="flex flex-row-reverse text-lg opacity-80">{user?.name ?? "Guest"}</h2>
