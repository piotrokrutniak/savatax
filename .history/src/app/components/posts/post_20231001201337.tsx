...
export function PostItem({post}:{post: Post}){
      ...
      return(
        <div className="w-full p-4 md:h-48 bg-black hover:bg-slate-500/20 rounded-lg shadow-md shadow-black/40 flex flex-col md:flex-row md:justify-between gap-8 items-center text-white group transition-all">
          <div className='h-64 w-full md:w-40 md:h-40 shrink-0 rounded-lg overflow-hidden relative cursor-pointer flex justify-center place-items-center bg-black'>
          ...
          <div className="h-fit items-center">
            <div className="flex flex-col w-full justify-between gap-5 text-xl">
              ...
              <div className="w-full flex-col">
                <h2 className="flex flex-row-reverse">{user?.name ?? "Guest"}</h2>
                <h2 className="flex flex-row-reverse opacity-60 font-thin">@{user?.username ?? "Guest"}</h2>
              </div>
            </div>
