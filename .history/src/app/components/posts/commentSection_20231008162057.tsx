    ...
    return(
      <div className="bg-slate-500/30 w-full overflow-hidden rounded-lg relative h-full">
        ...
        <div className={`${(inputActive || commentString.length > 0) ? "flex-col" : ""} p-4 border-t-2 relative border-slate-400/20 flex gap-2`}>
          <FormInput onFocus={HandleFocus} onBlur={HandleBlur} onChange={setCommentString} value={commentString} className="w-full" label={"Title"} placeholder="Start typing..."/>
          <Button onClick={SendComment} className="w-fit h-fit ml-auto mr-0"> Comment <BsSend className="aspect-square w-6"/> </Button>
        </div>
      </div>
    )
  }
