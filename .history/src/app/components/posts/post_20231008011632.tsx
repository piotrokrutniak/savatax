  ...
  function CommentsSection({post, setCommentsOpen}:{
    ...
    return(
      <div className="bg-slate-500/30 w-full relative h-full">
        ...
        <div className="p-4 min-h-screen-1/4 max-h-80 flex flex-col gap-4 bg-slate-500/30 overflow-y-scroll">
          {loading ?
          ...
          <div className="flex gap-2 select-none h-80 place-items-center bg-black p-8 rounded-md justify-center"> <p>Loading results</p> <BsArrowClockwise className="animate-spin h-6 w-6"/></div>}
        </div>
        <div className="p-4 border-t-2 relative border-slate-400/20 flex gap-4">
          <FormInput onChange={setCommentString} value={commentString} className="w-full" placeholder="Start typing..."/>
          <Button onClick={SendComment}> Comment <BsSend/> </Button>
        </div>
      </div>
    )
  }
    ...
    <div className="p-4 bg-black rounded-lg flex flex-col gap-1">
      <div className="flex flex-col gap-3">
        <div className="flex justify-between place-items-center p-2">
          ...
          <Button className="text-sm bg-transparent outline-2 outline-slate-400/30 hover:outline-slate-400/60 outline active:opacity-80" onClick={undefined}>
