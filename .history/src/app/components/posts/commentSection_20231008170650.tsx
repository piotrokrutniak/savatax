...
export default function CommentsSection({post, setCommentsOpen}:{
    ...
    return(
      <div className="bg-slate-500/30 w-full overflow-hidden rounded-lg relative h-full">
        ...
        <div className={`${(inputActive || commentString.length + titleString.length > 0) ? "flex-col" : ""} p-4 border-t-2 relative border-slate-400/20 flex gap-2`}>
          ...
          <FormInput onFocus={HandleFocus} onBlur={HandleBlur} onChange={setCommentString} value={commentString} 
            className={`${(commentString.length + titleString.length > 0) ? "block" : "hidden"} w-full`}
            placeholder="Enter title..."/>
