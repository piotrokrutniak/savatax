...
export function PostItem({post}:{post: Post}){
      ...
      useEffect(() => {
        const handler = (event: MouseEvent) => {
          if (!commentsRef.current?.contains(event.target.parentNode as Node)) {
        ...
        <div className="w-full p-4 bg-black hover:bg-slate-500/20 rounded-lg shadow-md shadow-black/40 flex flex-col md:flex-row md:justify-between gap-6 items-center text-white group transition-colors">
          ...
          <div className="h-fit items-center">
            ...
            <div className="flex relative flex-row-reverse items-center pt-5">
              ...
              {commentsOpen && 
              <div className="w-full absolute top-full h-fit rounded-lg mt-2 bg-black z-20 shadow-md shadow-black/40" ref={commentsRef} onBlurCapture={() => setCommentsOpen(false)}>
                <CommentsSection post={post} setCommentsOpen={setCommentsOpen} commentsRef={commentsRef}/>
              </div>}
            </div>
          </div>          
        </div>
