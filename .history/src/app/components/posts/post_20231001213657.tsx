...
export function PostItem({post}:{post: Post}){
      ...
      return(
        <div className="w-full p-0 md:p-4 bg-black hover:bg-slate-500/20 rounded-lg shadow-md overflow-clip shadow-black/40 flex flex-col md:flex-row md:justify-between gap-4 items-center text-white group transition-colors">
          <div className='md:h-full w-full aspect-square md:w-40 h-80 shrink-0 md:rounded-lg overflow-hidden relative cursor-pointer flex justify-center place-items-center bg-black shadow-sm shadow-black/40'>
            {//<Image src={image} layout="fill" objectFit="cover" alt="Breakfast Photo"/>
            }
            <FaImage className="w-20 h-20 md:w-12 opacity-30 flex"/>
          </div>
          <div className="h-fit items-center p-4 md:p-0">
            <div className="flex flex-col md:flex-row w-full justify-between text-xl">
              <h1 className="flex place-items-center cursor-pointer bg-clip-text w-full whitespace-break-spaces capitalize overflow-ellipsis overflow-hidden hover:text-transparent bg-gradient-to-r from-vermilion-500 to-vermilion-400">
