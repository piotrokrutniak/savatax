...
export default function Home(pageData: {params: {countryCode: keyof typeof Localization}, searchParams: any}) {
  ...
  return (
    ...
    <section className="w-full p-2 xs:p-4 md:p-8 -top-20 relative">
      <div className="max-w-6xl w-full relative mx-auto py-16">
        ...
        <div className='bg-blue-400 w-full h-4 hidden sm:block'></div>
...
function ServiceTab({label, index, activeIndex, setActiveIndex}:{label: string, index: number, activeIndex: number, setActiveIndex: Dispatch<SetStateAction<number>>}){
  return(
    <button onClick={() => setActiveIndex(index)} className={`${index == activeIndex ? "bg-blue-400  text-white" : "bg-blue-100/40 text-blue-400/80 hover:text-blue-400"} 
      lg:text-3xl text-lg sm:text-xl font-bold px-4 py-3 md:p-4 max-sm:rounded-b-lg rounded-t-lg transition-all max-sm:shadow-lg`}>
        {label}
    </button>
  )
}