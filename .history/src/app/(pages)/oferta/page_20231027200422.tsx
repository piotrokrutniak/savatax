...
function ServiceTab({label, index, activeIndex}:{label: string, index: number, activeIndex: number}){
  return(
    <button className={`${index == activeIndex ? "bg-blue-400  text-white" : "bg-slate-100 text-blue-400"} md:text-3xl pb-0 text-2xl font-bold  md:p-4 rounded-t-lg transition-all`}>{label}</button>
  )
}