  ...
  return (
    ...
    <section className="w-full p-4 md:p-8 -top-20 relative">
      <div className="max-w-6xl w-full relative mx-auto py-16">
        <h1 className="md:text-3xl text-2xl font-bold text-blue-400 md:p-4">{Localization[language].servicesPage.services[activeIndex].header}</h1>
        <ul className="flex flex-col gap-8 mt-4">
          {Localization[language].servicesPage.services[activeIndex].panels.map((x, key) => <DescPanel title={x.title} text={x.text} key={key}/>)}
        </ul>
      </div>
    </section>
    </>
