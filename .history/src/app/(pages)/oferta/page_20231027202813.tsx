...
function ServiceTab({label, index, activeIndex, setActiveIndex}:{label: string, index: number, activeIndex: number, setActiveIndex: Dispatch<SetStateAction<number>>}){
  return(
    <button onClick={() => setActiveIndex(index)} className={`${index == activeIndex ? "bg-blue-400  text-white" : "bg-blue-100/40 text-blue-400/80 hover:text-blue-400"} lg:text-3xl text-lg sm:text-xl font-bold px-4 py-3 md:p-4 max-sm:rounded-b-lg rounded-t-lg transition-all`}>{label}</button>
  )
}