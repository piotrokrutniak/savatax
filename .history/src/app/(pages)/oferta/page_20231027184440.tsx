...
export default function Home(pageData: {params: {countryCode: keyof typeof Localization}, searchParams: any}) {
  ...
  return (
    <>
    <section className="w-full h-full pt-16 lg:pt-32 -top-20 relative bg-blue-400 bg-gradient-to-t from-blue-400 to-blue-600 flex place-items-center shadow-lg">
      <div className="max-w-6xl w-full relative mx-auto lg:py-16 overflow-x-hidden">
        <div className="flex relative w-full h-128 lg:h-96 transition-all duration-300" style={{transform: `translateX(-${activeIndex * 100}%)`}}>
          {Localization[language].servicesPage.slider.map((x, key) => {
            return <OfferPanel slide={x} key={key} index={key} activeIndex={activeIndex}/>;
          })}
        </div>
        <div className='flex justify-center h-fit relative gap-2 pt-16 bottom-0 transition-all'>
    ...
    <section className="w-full -top-20 relative">
...
function OfferPanel({slide, index, activeIndex}:{slide: ServiceSlide, index: number, activeIndex: number}){
  return(
    <div className={`${activeIndex == index ? "opacity-100" : "opacity-0"} flex flex-col lg:flex-row absolute p-8 h-full w-full place-items-center gap-8 lg:gap-16 transition-all ease-in-out duration-300`} style={{left: `${index * 100}%`}}>
      <div className="h-96 w-96 rounded-lg bg-white relative aspect-square shadow-lg overflow-clip">
