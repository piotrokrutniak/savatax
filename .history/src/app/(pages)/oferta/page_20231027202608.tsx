...
export default function Home(pageData: {params: {countryCode: keyof typeof Localization}, searchParams: any}) {
  ...
  return (
    ...
    <section className="w-full p-1 xs:p-4 md:p-8 -top-20 relative">
...
function ServiceTab({label, index, activeIndex, setActiveIndex}:{label: string, index: number, activeIndex: number, setActiveIndex: Dispatch<SetStateAction<number>>}){
  return(
    <button onClick={() => setActiveIndex(index)} className={`${index == activeIndex ? "bg-blue-400  text-white" : "bg-blue-100/40 text-blue-400/80 hover:text-blue-400"} lg:text-3xl text-lg sm:text-xl font-bold px-4 py-3 md:p-4 rounded-t-lg transition-all`}>{label}</button>
  )
}