...
function OfferPanel({slide, index, activeIndex}:{slide: ServiceSlide, index: number, activeIndex: number}){
  return(
    <div className={`${activeIndex == index ? "opacity-100" : "opacity-0"} flex absolute p-8 h-full w-full place-items-center gap-16 transition-all ease-in-out duration-300`} style={{left: `${index * 100}%`}}>
      ...
      <div className="flex flex-col text-white gap-4">
        ...
        <ul className="text-xl flex flex-col gap-2 ">
          {slide.bullets.map((x, key) => <li className="flex place-items-start" key={key}><BsArrowRightShort className="h-8 w-8 mb-1 shrink-0"/> {x}</li>)}
        </ul>
      </div>
    </div>
  )
