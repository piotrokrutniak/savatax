...
function ServiceTab({label, index, activeIndex, setActiveIndex}:{label: string, index: number, activeIndex: number, setActiveIndex: Dispatch<SetStateAction<number>>}){
  return(
    <button onClick={() => setActiveIndex(index)} className={`${index == activeIndex ? "bg-blue-400  text-white" : "bg-blue-100/40 text-blue-400/80 hover:text-blue-400"} md:text-3xl text-2xl font-bold px-4 md:p-4 rounded-t-lg transition-all`}>{label}</button>
  )
}