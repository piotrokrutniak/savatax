...
export default function UserPage(pageData: {params: {id: number}, searchParams: any}){
    ...
    return(
        ...
        <section id="header-section" className='max-w-7xl flex flex-col overflow-hidden gap-5 bg-black/90 m-auto w-full rounded-xl mt-4 shadow-md shadow-black/40 text-white p-4 sm:p-8 relative'>
            <div id="photo-section" className="w-full h-full sm:h-screen/2 aspect-square relative">
                <Image layout="fill" objectFit="contain" src={selectedPhoto?.url ?? ""} alt="Album thumbnail" className="aspect-square h-full"/>
            </div>
            <div id="user-info" className="flex max-xs:justify-between flex-row-reverse gap-5">
            ...
            <div className="text-xl md:text-3xl font-semibold px-4 sm:px-8">Photos</div>
            <div id="gallery" className="grid gap-2 p-1 sm:gap-4 grid-auto-fit-mobile sm:grid-auto-fit-xs sm:p-4 shadow-inner shadow-black/40 rounded-lg bg-slate-500/30">
...
function GalleryPanel({photo, setPhoto}:{
    ...
    return(
        <div className="relative aspect-square">
            <Image layout="fill" objectFit="contain" src={photo.url ?? ""} alt="Album thumbnail" className="max-h-96 h-full cursor-pointer active:opacity-80 transition-all" onClick={() => setPhoto(photo)}/>
        </div>
    )
}