...
export default function UserPage(pageData: {params: {id: number}, searchParams: any}){
    ...
    return(
        ...
        <section id="header-section" className='max-w-7xl flex flex-col overflow-hidden gap-5 bg-black/90 m-auto w-full rounded-xl mt-4 shadow-md shadow-black/40 text-white p-8 relative'>
            <div className="w-full max-h-96 h-auto relative">
                <Image layout="fill" objectFit="contain" src={selectedPhoto?.url ?? ""} alt="Album thumbnail"/>
            </div>
            <div className="flex max-sm:justify-between flex-row-reverse gap-5">
