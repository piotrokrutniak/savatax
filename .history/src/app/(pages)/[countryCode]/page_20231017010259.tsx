...
import { CountryCode, Localization } from '@/app/types'

export default function Home(pageData: {params: {countryCode: keyof typeof Translations}, searchParams: any}) {
  const language = pageData.params.countryCode
  ...
  return (
    <div className="flex flex-col relative -top-20 gap-32 scroll-smooth">
      <section className="w-full h-screen-3/5 bg-gradient-to-t top-0 from-black to-black/90 relative text-white">
        <div id="slider-section" className="absolute  max-md:px-4 py-8 w-full mx-auto z-10 m-auto bottom-0 shadow-md shadow-black/40 h-full flex place-items-end lg:place-items-center">
            <div className='max-w-6xl md:px-16 mx-auto w-full rounded-xl flex flex-col gap-4'>
              <h1 className="font-bold md:w-1/2 text-xl sm:text-2xl lg:text-4xl max-w-6xl">{Translations[language].lblSliderHeader}</h1>
              <h2 className="opacity-90 lg:w-1/2 sm:text-lg md:text-xl max-w-6xl md:leading-8">
                {Translations[language].lblSliderP1}
                <br/>
                {Translations[language].lblSliderP2}
              </h2>
              ...
              <Button className="w-fit md:text-xl p-4 mt-2 shadow-md shadow-black/20 active:opacity-80
                ...
                {Translations[language].lblContact}
              </Button>
              </Link>
            </div>
        </div>
      ...
      <section id="summary-section" className='max-w-6xl p-4 sm:p-8 mx-auto w-full justify-center md:px-16 rounded-xl flex flex-col gap-5'>
            <h1 className="md:text-3xl text-2xl font-bold text-blue-400 md:p-4">{Translations[language]['section-1'].header}</h1>
            <p className="md:leading-8 md:text-lg font-normal ">
              {Translations[language]['section-1']['paragraph-1']}
            </p>

            <p className="md:leading-8 md:text-lg opacity-90">
              {Translations[language]['section-1']['paragraph-2']}
            </p>

            <p className="pt-16">
              <span className="md:leading-8 text-xl md:text-2xl font-semibold opacity-90 md:p-4">
              {Translations[language]['section-2']['header']}
              </span>
              <ul className="md:text-lg p-2 md:p-12 my-12 rounded-lg shadow-inner gap-6 grid grid-auto-fit-md bg-sky-100/30">
                <ListPanel> 
                  <FaBook className="w-8 h-8 fill-blue-400 shrink-0 mt-1"/> <p className="capitalize-first">{Translations[language]['section-2']['cards']['card-1']}</p>
                </ListPanel>
                <ListPanel> 
                  <FaFileInvoiceDollar className="w-8 h-8 fill-blue-400 shrink-0 mt-1"/> <p className="capitalize-first">{Translations[language]['section-2']['cards']['card-2']}</p>
                </ListPanel>
                <ListPanel> 
                  <FaPiggyBank className="w-8 h-8 fill-blue-400 shrink-0 mt-1"/> <p className="capitalize-first">{Translations[language]['section-2']['cards']['card-3']}</p>
                </ListPanel>
                <ListPanel> 
                  <FaChartPie className="w-8 h-8 fill-blue-400 shrink-0 mt-1"/> <p className="capitalize-first">{Translations[language]['section-2']['cards']['card-4']}</p>
                </ListPanel>
                <ListPanel> 
                  <FaBalanceScale className="w-8 h-8 fill-blue-400 shrink-0 mt-1"/> <p className="capitalize-first">{Translations[language]['section-2']['cards']['card-5']}</p>
                </ListPanel>
              </ul>
            </p>

            <p className="md:text-lg" >
              {Translations[language]['section-3']['paragraph-1']}
            </p>
            <p className="md:text-lg" >
              {Translations[language]['section-3']['paragraph-2']}
            </p>

            <p className="md:text-xl font-semibold pt-8" >
              {Translations[language]['section-3']['paragraph-3']}
            </p>        
      </section>
      <section id="contact-section" className='max-w-6xl p-4 sm:p-8 mx-auto w-full justify-center md:px-16 rounded-xl flex flex-col gap-5'>
        <div className="flex justify-between place-items-center">
          <h1 className="md:text-3xl text-2xl font-bold text-blue-400 md:p-4"> {Translations[language]['section-contact']['header']}</h1>
          <Link href={"mailto:karol.zach@savatax.com"}>
            <h2 className="md:text-xl text-lg text active:opacity-80 border-b-2 pb-0 border-black hover:border-blue-400 font-semibold hover:text-blue-400 cursor-pointer transition-all flex">
              {Translations[language]['section-contact']['headerButton']} <FaEnvelope className="ml-2 mt-1"/> 
            </h2>
          </Link>
        </div>
